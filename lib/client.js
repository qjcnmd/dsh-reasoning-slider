window.__ModuleLoader__.load({
	...
	factory: (require) => {
      ...
      .rsm-rail { position: absolute; left: -12px; right: -12px; top: 4px; height: 24px; border-radius: 12px; background: #E7E7E7; }
      .rsm-fill { position: absolute; z-index: 0; left: -12px; top: 4px; height: 24px; border-radius: 12px; background: #4F95F8; box-shadow: inset 0 1px 1px rgba(255,255,255,.18), inset 0 -2px 4px rgba(28,53,112,.2), inset 0 0 7px rgba(25,45,100,.14); overflow: hidden; transition: width .22s cubic-bezier(.2,.8,.3,1); }
      ...
      .rsm-energy { position: absolute; z-index: 1; inset: 0; overflow: hidden; background-image: radial-gradient(circle at 8% 26%, rgba(255,255,255,.72) 0 1px, transparent 2px), radial-gradient(circle at 24% 72%, rgba(210,232,255,.58) 0 1px, transparent 2px), radial-gradient(circle at 41% 35%, rgba(255,255,255,.72) 0 1px, transparent 2px), radial-gradient(circle at 59% 79%, rgba(213,227,255,.55) 0 1px, transparent 2px), radial-gradient(circle at 76% 21%, rgba(255,255,255,.66) 0 1px, transparent 2px), radial-gradient(circle at 92% 64%, rgba(225,215,255,.76) 0 1px, transparent 2px); background-repeat: repeat-x; background-size: 62px 24px, 78px 24px, 94px 24px, 70px 24px, 86px 24px, 108px 24px; animation: rsm-energy-drift 1.9s linear infinite; pointer-events: none; }
      .rsm-fill-max .rsm-energy { background-image: radial-gradient(circle at 8% 26%, rgba(255,255,255,.86) 0 1px, transparent 2px), radial-gradient(circle at 24% 72%, rgba(226,215,255,.7) 0 1px, transparent 2px), radial-gradient(circle at 41% 35%, rgba(255,255,255,.86) 0 1px, transparent 2px), radial-gradient(circle at 59% 79%, rgba(231,211,255,.68) 0 1px, transparent 2px), radial-gradient(circle at 76% 21%, rgba(255,255,255,.8) 0 1px, transparent 2px), radial-gradient(circle at 92% 64%, rgba(242,222,255,.9) 0 1px, transparent 2px); }
      .rsm-fill-max .rsm-energy { animation-duration: .98s; animation-timing-function: linear; }
      .rsm-energy::after { content: ""; position: absolute; top: -5px; bottom: -5px; width: 13%; background: linear-gradient(105deg, rgba(255,255,255,0), rgba(255,255,255,.08) 30%, rgba(255,255,255,.3) 50%, rgba(226,239,255,.1) 70%, rgba(255,255,255,0)); filter: blur(.6px); animation: rsm-energy-glint 2.1s cubic-bezier(.32,.08,.28,1) infinite; pointer-events: none; }
      .rsm-fill-max .rsm-energy::after { background: linear-gradient(105deg, rgba(255,255,255,0), rgba(255,255,255,.13) 30%, rgba(255,250,255,.76) 50%, rgba(231,206,255,.2) 70%, rgba(255,255,255,0)); }
      .rsm-fill-max .rsm-energy::after { animation-duration: 1.15s; animation-timing-function: linear; }
      ...
      .rsm-fill::before { content: ""; position: absolute; z-index: 3; inset: 0; border-radius: inherit; background: linear-gradient(90deg, rgba(19,35,83,.18), transparent 18%, transparent 76%, rgba(39,15,92,.28)), linear-gradient(180deg, rgba(12,24,62,.1), transparent 42%, rgba(12,24,62,.2)); box-shadow: inset 0 0 5px rgba(16,24,64,.28); pointer-events: none; }
      ...
      .rsm-tick { position: absolute; z-index: 1; top: 14px; width: 4px; height: 4px; margin-left: -2px; border-radius: 50%; background: #c2c6cc; }
      .rsm-tick-on { background: rgba(224,241,255,.78); }
      .rsm-tick-major { top: 12px; width: 8px; height: 8px; margin-left: -4px; }
      .rsm-thumb { position: absolute; z-index: 2; top: 2.5px; width: 27px; height: 27px; margin-left: -13.5px; border-radius: 50%; background: #fff; border: 1px solid rgba(20,30,50,.1); box-shadow: 0 1px 2px rgba(20,30,50,.12), 0 3px 8px rgba(20,30,50,.08); box-sizing: border-box; transition: left .18s cubic-bezier(.2,.9,.25,1); }
    ...
    `;
		...
		function EffortSlider({ efforts, index, onChange, onSubmit, disabled }) {
			...
			const magnetize = (position) => {
				...
				return Math.abs(position - nearest) <= 0.18 ? nearest : clamp(position);
			};
			const down = (e) => {
				if (disabled) return;
				e.preventDefault();
			...
			return React.createElement("div", {
				...
				React.createElement("div", { className: "rsm-range" },
					...
					React.createElement("div", { className: "rsm-fill" + (atMax ? " rsm-fill-max" : "") + (dragging ? " rsm-fill-drag" : "") + (ultraBurst ? " rsm-ultra-burst" : ""), style: { width: "calc(" + leftPct + "% + 12px)" } },
					...
					efforts.map((effort, i) => React.createElement("div", {
						key: effort.id,
						className: "rsm-tick" + (i === 1 ? " rsm-tick-major" : "") + (i <= shown ? " rsm-tick-on" : ""),
